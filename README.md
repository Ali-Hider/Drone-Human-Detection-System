...
`Dataset Analysis` $\rightarrow$ `Preprocessing` $\rightarrow$ `Model Training` $\rightarrow$ `Inference` $\rightarrow$ `Counting Logic` $\rightarrow$ `Evaluation`
...
- **Resizing:** All images are resized to 640x640 pixels.
- **Normalization:** Pixel values are scaled to [0, 1].
- **Dynamic Augmentation:** Utilizing YOLOv8's internal pipeline for Mosaic Augmentation, Horizontal Flipping, and Rotation/Scaling.

### 3. Engineering Challenges Identified
- **The Small Object Problem:** Targets occupy very few pixels, increasing the risk of False Negatives.
- **High Aspect Ratio:** Extreme widths can cause distortion during square resizing.
- **Class Imbalance:** Higher frequency of vehicle detections compared to pedestrians.

### 4. Visualizations
*(Visualizations are provided in the `/visualizations` folder and the Colab notebook)*
- Raw Data Visualization
- Filtered Target Class Visualization
- Preprocessing Proof Grid
